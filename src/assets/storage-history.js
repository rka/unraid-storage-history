(function () {
  ...
  function dotGraph(values) { const glyphs = '⣀⣀⣄⣆⣇⣧⣷⣿'; const max = Math.max(...values, 1); return '<span class="sh-dotgraph">'+values.map(value => glyphs[Math.min(glyphs.length - 1, Math.round(value / max * (glyphs.length - 1)))]).join('')+'</span>'; }
