(function () {
  ...
  function dotGraph(values) { const glyphs = '⣀⣀⣄⣆⣇⣧⣷⣿'; const max = Math.max(...values, 1); return '<span class="sh-dotgraph">'+values.map(value => '<i>'+glyphs[Math.min(glyphs.length - 1, Math.round(value / max * (glyphs.length - 1)))]+ '</i>').join('')+'</span>'; }
  function render(root, payload, compact) {
    ...
    root.innerHTML = (compact ? '' : '<div class="sh-ranges">'+['24h','7d','30d','90d','1y','all'].map(r => '<button data-range="'+r+'" class="'+(r===payload.range?'active':'')+'">'+r+'</button>').join('')+'</div>')+
      graph(payload.samples) + '<div class="sh-stats"><div><span>Used</span><strong>'+bytes(current.used)+'</strong></div><div><span>Free</span><strong>'+bytes(current.free)+'</strong></div><div><span>Total</span><strong>'+bytes(current.total)+'</strong></div><div><span>Growth</span><strong>'+growth(payload.growth_per_day)+'</strong></div></div><div class="sh-live"><span class="sh-live-title">Disk I/O</span><span class="sh-io sh-io-read">↓ '+bytes(io.read_per_second)+'/s '+dotGraph(history.read)+'</span><span class="sh-io sh-io-write">↑ '+bytes(io.write_per_second)+'/s '+dotGraph(history.write)+'</span></div>';
